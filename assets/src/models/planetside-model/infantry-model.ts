import { MeshPhongMaterial, MeshLambertMaterial, MeshBasicMaterial, Vector3, LoadingManager } from 'three';
...
export class InfantryModel extends PlanetsideModel {
  ...
  private setCharacterWeapon = () => {
    this.characterWeapon =
      this.characterFactionAlias + '_' + this.CHARACTER_WEAPON_MAP.get(this.characterClass) + '_Weapon';
  };
  ...
  private getNonNullCharacterVariables = (factionAlias: string, headID: number, characterClass: string) => {
    ...
    if (
      ...
      ![...this.CHARACTER_CLASS_MAP.keys()].includes(characterClass)
    ) {

...
export class FlexBootstrapTableFilter {
  ...
  private addCustomListenersPlease = () => {
  ...
  private updateEachOptionAvailability = (filterCategory: string, filterOptions: TableFilter[]) => {
    ...
    filterOptions.forEach((filter) => {
      ...
      if (input != undefined) {
        ...
        (
          input.parentElement?.querySelector('span')?.querySelector('.filter-option-contains') as HTMLElement
        ).innerHTML = '(' + newArraySize + ')';
      }
    });
  };
  ...
  private defaultFiltrationFunction = (
    filterCategory: string,
    filterOptions: TableFilter[],
    dataArray: ITableData[],
  ) => {
    //initialize variables
  ...
  public getOriginalTableData = () => {
    return this.hasID
      ? JSON.parse(JSON.stringify(this.idFilteredData))
      : JSON.parse(JSON.stringify(this.originalTableData));
  };
